...
class ChatBot:
    ...
    def gpt3_completion(self, prompt, engine='text-davinci-003', temp=0.7, top_p=1.0, tokens=400, freq_pen=0.0, pres_pen=0.0, stop=['User:', 'Assistant:']):
        max_retry = 5
        retry = 0
        ...
        while True:
            try:
                ...
                self.file.save_file('gpt3_logs/%s' % filename,
                                    prompt + '\n\n==========\n\n' + text)
                return text
    ...
    def summarize_memories(self, conn, memories):
      # Summarizes a block of memories into one payload.
        ...
        for memory in memories:
            message_block += memory['role'].upper() + \
                ': ' + memory['message'] + '\n\n'
        ...
        if message_block == '':
            ...
            with open('prompt/prompt_notes.txt', 'r') as f:
        ...
        uuid = str(uuid4())
        timestamp = time()
        note_data = (uuid, notes, timestamp, uuids,
                     timestamp_str, serialized_vector)
    ...
    def store_chatlog(self, conn, message, role):
        timestamp = time()
        vector = self.gpt3_embedding(message)
        serialized_vector = pickle.dumps(vector)
        uuid = str(uuid4())
        chatlog = (uuid, role, timestamp, message, serialized_vector)
    ...
    def ask_chatgpt_question(self, question):
        ...
        database = os.path.join(os.getcwd(), self.db_folder, self.db_name)

        conn = self.create.create_connection(database)

        # question = self.get_last_message(conn)
        vector = self.store_chatlog(conn, question, role='user')
        ...
        prompt = self.file.open_file('prompt/prompt_response.txt').replace(
            '<<INVENTORY>>', inventory).replace('<<NOTES>>', notes).replace('<<CONVERSATION>>', recent)
        ...
        cleaned_lst = [x for x in collected_messages if x is not None]
        result = ''.join(cleaned_lst)
        # Insert chatbot's answer into chat_log table
        self.store_chatlog(conn, result, role='assistant')
